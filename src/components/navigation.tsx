import { Moon, Sun, Sparkles, Menu, X, Zap, Palette } from 'lucide-react';
    ...
    <>
      ...
      >
        <div className={cn(
          ...
          isScrolled && 'bg-background/80 backdrop-blur-2xl border border-border/50 rounded-2xl shadow-2xl shadow-black/5 mx-4 sm:mx-6 lg:mx-8 mt-2'
        )}>
          <nav className="flex h-14 items-center justify-between">
            <motion.a href="#" className="flex items-center gap-3 group" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
              <motion.div className="flex items-center justify-center w-10 h-10 rounded-xl bg-gradient-to-br from-linear to-purple-600 text-white shadow-lg shadow-linear/25"
                whileHover={{ rotate: [0, -10, 10, 0] }} transition={{ duration: 0.5 }}>
                <Sparkles className="w-5 h-5" />
              </motion.div>
              <span className="font-display font-bold text-xl hidden sm:block">UI Hub</span>
            </motion.a>
            ...
            <div className="flex items-center gap-3">
              <MagneticButton onClick={() => setTheme(resolvedTheme === 'dark' ? 'light' : 'dark')}
                className={cn('flex items-center justify-center w-10 h-10 rounded-full transition-all', 'bg-secondary hover:bg-secondary/80 text-foreground border border-border/50')}
                strength={0.3}>
              ...
              <motion.button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="md:hidden flex items-center justify-center w-10 h-10 rounded-full bg-secondary hover:bg-secondary/80 transition-colors" whileTap={{ scale: 0.95 }}>
                <AnimatePresence mode="wait">
                  {isMobileMenuOpen ? (
              ...
              <div className="flex flex-col gap-2">
                {navItems.map((item, index) => (
                  <motion.a key={item.name} href={item.href} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.1 }} onClick={() => setIsMobileMenuOpen(false)}
                    ...
                    <item.icon className="w-5 h-5 text-linear" />
                    {item.name}
                  </motion.a>
                ))}
              </div>
